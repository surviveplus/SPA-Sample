...
$().ready(() => {
    ...
    var search = (keyword: string) => {
        ...
        var contains = (text: string, keyword: string) => {
            return (text ? text.indexOf(keyword) >= 0 : false);
        };

        var keywords = $("#keywords").val();
        if (keywords != "" && keywords) {
            keywords.split(" ").forEach((keyword, i, a) => {
                targets = targets.Where((item, index) => {
                    return (
    ...
    $("#keywords").change((e) => {
        ...
        this.Search(changed.val());
    });
    var searchTimerToken = setInterval(() => {
        search($("#keywords").val());
    }, 600);
